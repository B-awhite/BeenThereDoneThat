...
function addPlace(e) {
    ...
    const imgInput = e.target.children[2]
...
function fetchPlaces() {
    ...
    .then(data => {
        ...
        // addPlaceEvents()
        const place = document.querySelectorAll(".place-delete-button")
       place.forEach(pl => pl.addEventListener("click", deletePlace))
    })
}
    ...
    return `
      <div class="place-card" id="${place.id}">
         <div class="place-frame" id="${place.id}">
            ...
            <img src= ${place.img} height=200 width=200> 
...
function addCreateForm() {
    ...
    form.innerHTML = `<input id="name-input" placeholder='Place' type='text'/><input id="location-input" placeholder='City' type='text'/><input id="img-input" placeholder="Image" type='text'/><input id="place-submit" value="Create Place" type='submit'/>`
